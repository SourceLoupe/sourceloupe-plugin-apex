...
import TsSfApex from 'tree-sitter-sfapex';export function getFieldValuesForQuery(
    treeSitterQuery: string,
    _fieldName: string,
    startFromNode: Parser.SyntaxNode
): Map<string, Parser.SyntaxNode[]> {
    const resultMap = new Map<string, Parser.SyntaxNode[]>();    const query = new TreeSitter.Query(TsSfApex.apex, treeSitterQuery);
