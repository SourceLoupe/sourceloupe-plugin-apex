...
@suggestion("")
@priority(3)
@query('(method_invocation object:(identifier) @object (#match? @object "Crypto") name: (identifier) @method (#match? @method "encrypt|decrypt") (argument_list)@args)')
@regex("")
export class ApexBadCrypto extends ScanRule {
    validateNode(node: Parser.SyntaxNode): ScanResult[] {
        ...
        const allGoodCryptoCalls: string = '(variable_declarator name:(identifier) @varname value: (method_invocation object:(identifier) @object (#match? @object "Crypto") name: (identifier) @method (#match? @method "generateAesKey|encryptWithManagedIV")))';
        const allEncryptDecryptCalls: string = '(method_invocation object:(identifier) @object (#match? @object "Crypto") name: (identifier) @method (#match? @method "encrypt|decrypt") (argument_list) @args)';
        ...
        encryptDecryptCallList.forEach(encryptDecryptIteration=>{
            ...
            const ivNode: Parser.SyntaxNode = encryptDecryptIteration.node.namedChildren[2];
