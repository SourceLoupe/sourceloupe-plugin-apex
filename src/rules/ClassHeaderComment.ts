...
export class ClassHeaderComment extends ScanRule {
    validateNode(node: Parser.SyntaxNode): ScanResult[] {
        const beforeClassNode: Parser.SyntaxNode = node.previousNamedSibling ?? node;
        if (beforeClassNode.grammarType == 'block_comment' && beforeClassNode.text.includes('description')) {
            return [];
        }
